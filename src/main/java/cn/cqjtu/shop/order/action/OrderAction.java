package cn.cqjtu.shop.order.action;
...
import cn.cqjtu.shop.utils.PaymentUtil;


public class OrderAction extends ActionSupport implements ModelDriven<Order> {
	private Order order = new Order();
	private OrderService orderService;
	private Integer page;// 接收显示页数
	private String pd_FrpId;// 付款通道
	// 接收付款成功后的参数:
	...
	public String addOrder() {
		...
		order.setState(1);// 订单状态,1.未付款 ,2.已付款未发货,3.已发货未收货,4.交易完成
		HttpSession session = ServletActionContext.getRequest().getSession();
		Cart cart = (Cart) session.getAttribute("cart");
		if (cart == null) {
			this.addActionError("购物车为空，订单提交失败！");
			return "msg";
		}
	...
	public String payOrder() throws IOException {
		...
		String p8_Url = "http://172.16.105.75:8080/SSH-Shop/order_callBack.action"; // 商户接收支付成功数据的地址:
		String p9_SAF = ""; // 送货地址:
		String pa_MP = ""; // 商户扩展信息:
