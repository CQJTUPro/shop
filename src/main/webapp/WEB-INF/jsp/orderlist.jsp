			...
			<table align="center">
				<tbody>
					<s:iterator value="pageBean.list" var="order">
						<tr>
							<td colspan="5" bgcolor="#dff0d8">订单编号：<s:property value="#order.oid" />&nbsp;&nbsp;&nbsp;&nbsp;
								订单状态： <s:if test="#order.state==1">
									<a
										...
										color="red">未付款</font></a>
								</s:if> <s:if test="#order.state==2">已付款</s:if> <s:if test="#order.state==3">
									<a
										href="${pageContext.request.contextPath}/order_updateState.action?oid=<s:property value="#order.oid" />"><font
										color="blue">确认收货</font></a>
								</s:if> <s:if test="#order.state==4">交易完成</s:if>&nbsp;&nbsp;&nbsp;&nbsp; <a
								href="${pageContext.request.contextPath}/order_findByOid.action?oid=<s:property value="#order.oid" />">查看详情</a></td>
						</tr>
